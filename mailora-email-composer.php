...
// ---------------------------------------------------------------------------
// 4b. Sanitize a raw "Name <email>" or plain "email" field.
//     Accepts an already-unslashed string; applies sanitize_text_field to the
//     display name and sanitize_email to the address, preserving the format.
// ---------------------------------------------------------------------------
function wpped_sanitize_address_field( $raw ) {
        $input = trim( $raw );
...
function wpped_handle_submission() {
        ...
        // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- wpped_sanitize_address_field applies sanitize_text_field on the display name and sanitize_email on the address
        $from_raw = wpped_sanitize_address_field( wp_unslash( $_POST['wpped_from'] ?? '' ) );
        // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- wpped_sanitize_address_field applies sanitize_text_field on the display name and sanitize_email on the address
        $to_raw   = wpped_sanitize_address_field( wp_unslash( $_POST['wpped_to']   ?? '' ) );
        $subject  = sanitize_text_field( wp_unslash( $_POST['wpped_subject'] ?? '' ) );
        // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- wpped_process_body applies wp_kses_post internally
        ...
        wpped_update_setting( 'from_address', $from_raw );
...
function wpped_render_page() {
        ...
        // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- wpped_sanitize_address_field applies sanitize_text_field on the display name and sanitize_email on the address
        $from    = $post_verified && isset( $_POST['wpped_from'] ) ? wpped_sanitize_address_field( wp_unslash( $_POST['wpped_from'] ) ) : wpped_get_setting( 'from_address' );
        // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- wpped_sanitize_address_field applies sanitize_text_field on the display name and sanitize_email on the address
        $to      = $post_verified && isset( $_POST['wpped_to'] )   ? wpped_sanitize_address_field( wp_unslash( $_POST['wpped_to'] ) )   : '';
        $subject = $post_verified && isset( $_POST['wpped_subject'] ) ? sanitize_text_field( wp_unslash( $_POST['wpped_subject'] ) ) : '';
        // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- wpped_process_body applies wp_kses_post internally
        $body    = $post_verified && isset( $_POST['wpped_body'] )    ? wpped_process_body( wp_unslash( $_POST['wpped_body'] ) )      : '';
        ...
        <div class="wrap">
                ...
                <?php else : ?>
                                        ...
                                        <table class="form-table" role="presentation">
                                                ...
                                                <tr>
                                                        ...
                                                        <td>
                                                                ...
                                                                <p class="description">Use your domain or SMTP-authorized email. Accepts <code>email@example.com</code> or <code>Name &lt;email@example.com&gt;</code>. Saved automatically after each send.</p>
                                                        </td>
                                                </tr>
                                                ...
                                                <tr>
                                                        ...
                                                        <td>
                                                                <input type="text" id="wpped_subject" name="wpped_subject"
                                                                        ...
                                                                        class="regular-text" style="width:100%;" required />
                                                                <p class="description">
                                                                        For Gmail replies, use the exact original subject. Add <code>Re: </code>&nbsp;at the beginning if missing to keep the conversation together.
                                                                </p>
                                                        </td>
                                                </tr>
                        ...
                        <ul>
                                <li>Emails are sent via <code>wp_mail()</code> as HTML. Some hosting providers may block or limit outgoing emails sent through wp_mail().</li>
